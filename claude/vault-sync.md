...
1. If `mode == "enable"`:
   - If `.vault-sync.toml` does NOT exist in the project root: analyze the project's documentation
     ...
     `20_Projects/[Project_Name]/03_AI_Workflow`, `10_Wiki_Knowledge/Domains`). Set transient or
...
3. If `enabled == false` AND `mode != "disable"`: output the warning
   `"LLM Wikiê°€ ë¹„í™œ?±í™” ?íƒœ?…ë‹ˆ?? vault-sync enable ë¡??œì„±?”í•˜?¸ìš”."` and HALT immediately.
...
- Fallback only if no tag is present: infer from explicit Single Source of Truth (SSOT) markers / a status table (a fallback only ??per `_knowledge-architecture.md Â§2`, a bare `status:` is no longer a standalone classifier).

...
- "LLM Wiki가 비활성화 상태입니다. `vault-sync enable` 로 활성화하세요."

**mode == "disable"**: `enabled: false` 로 업데이트 후 종료.

**mode == "status"**: enabled 여부 무관하게 항상 실행 (읽기 전용).

### Step 1 — 준비 (enabled == true 일 때)
...
각 `.md` 파일을 아래 기준으로 분류하여 볼트 엔트리 생성:
...
- 기존 볼트 파일 있으면 덮어쓰지 않고 병합
- YAML frontmatter 필수 (title, date, tags, source)
- 관련 개념은 `[[문서명]]` 양방향 링크
- `20_Projects/` 인덱스: 링크 진입점만 (산출물 내용 복사 금지)
- `000_Index.md` 에 새 엔트리 `[[문서명]]` 추가

### Step 3 — refactor

원본 소스 문서의 일반 개념 설명을 볼트 링크로 대체:
...
제외: `progress.md`, `plan.md`, `_README.md`, 이미 변환된 파일

### Step 4 — status (항상 실행 가능)

변경 없이 동기화 대상/제외/refactor 대상 목록만 출력.

### 절대 금지

- `20_Projects/` 에 plan.md, progress.md 생성 금지
- `99_System_Agent/` 수정 금지
- `40_Templates/` 원본 수정 금지
- 소스 문서 고유 내용 삭제 금지
- 양쪽 중복 내용 생성 금지
